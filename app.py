...
@app.route("/worker/<int:member_id>/history/excel")
def worker_history_excel(member_id):
    if session.get("system_role") not in ("supervisor", "admin"):
        return redirect(url_for("login"))
    cursor = get_cursor()
    cursor.execute("SELECT * FROM team_members WHERE id=%s", (member_id,))
    ...
    cursor.execute("""
        SELECT oc.id AS order_id, oc.created_at AS order_date, oc.status,
               p.name AS product_name, ps.size, oi.quantity,
               uu.status AS uniform_status, uu.return_note,
               COALESCE(
                   (SELECT COALESCE(ue2.custom_reason, ue2.reason) FROM uniform_exchanges ue2 WHERE ue2.uniform_id = uu.id ORDER BY ue2.id DESC LIMIT 1),
                   ''
               ) AS exchange_reason
        FROM order_items oi
        JOIN order_carts oc ON oi.cart_id = oc.id
    ...
    for u in rows:
        uniform_status = u["uniform_status"] or ""
        if uniform_status == "exchanged":
            status_label = "Exchanged"
            reason = (u["exchange_reason"] or "").replace("_", " ").title()
        elif uniform_status.startswith("returned_"):
            status_label = "Returned"
            reason = uniform_status.replace("returned_", "").replace("_", " ").title()
            if u["return_note"]:
                reason += f" — {u['return_note']}"
        elif uniform_status in ("lost", "stolen"):
            status_label = uniform_status.title()
            reason = ""
        elif uniform_status == "active":
            status_label = "Active"
            reason = ""
        else:
            status_label = (u["status"] or "").title()
        ...
        data.append({
            ...
            "Size": u["size"],
            "Qty": u["quantity"],
            "Date": u["order_date"].strftime("%d %b %Y") if u["order_date"] else "—",
            "Status": status_label,
            "Reason": reason or "—",
        })
    ...
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        df.to_excel(writer, index=False, sheet_name="Uniform History")
        ws = writer.sheets["Uniform History"]
        for col in ws.columns:
            max_len = max(len(str(cell.value or "")) for cell in col) + 4
            ws.column_dimensions[col[0].column_letter].width = min(max_len, 40)
    output.seek(0)

    filename = f"Uniform_History_{member['full_name'].replace(' ', '_')}_{member['employee_number']}.xlsx"
